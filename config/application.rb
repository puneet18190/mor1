...
module Mor
  class Application < Rails::Application
    ...
    # automatic attribute protection
    ...
    # Getting rid of "invalid byte sequence in UTF-8"
  ...
  config.after_initialize do
    ActiveProcessor.configure do |config|
      config.host = -> { Web_URL }
      config.translate_func = lambda { |s| _(s) }
      config.currency_exchange = lambda { |c1,c2| Currency.count_exchange_rate(c1, c2) }
      config.calculate_tax = lambda { |u, a| User.where({ :id => u}).first.get_tax.apply_tax(a) }
      config.substract_tax = lambda { |u, a| User.where({ :id => u}).first.get_tax.count_amount_without_tax(a) }
    end
  end
  end
end
