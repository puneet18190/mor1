...
(function($) {
    ...
    $.fn.callshop = function(options, i18ne) {
        ...
        $(this).each(function() {
            ...
            /* New Functionality Single Click Releases and starts new session right away */
            $(this).find("a.reset-session").live('click', function(ev){
                ev.preventDefault();
                callshop.reset_booth( $(this).closest('.booth') );
            });
            ...
            $("div.btn a.end-session").live('click', function(ev) {
                ...
                if (booth.state == "occupied") {
                    $(this).jConfirmAction({
                        ...
                        cancelAnswer: i18n.confirm.cancel,
                        callback: function() {
                            callshop.free_booth( booth.element  );
                        }
                    });
                } else {
            ...
            /* Quick New end session button
            No confirmation from user is needed */
            $("a.quick-end-session").live('click', function(ev) {
                ev.preventDefault();
                var booth = elementToBooth($(this).closest('.booth'));
            ...
            /*
             generates invoice' and resets session In single click.
            */
            $(this).find("input.generate_reset_booth").live('click', function() {
                var booth = $(this);
                var with_invoice = 1

                callshop.reset_booth(booth.closest('tr.slide').prev(), with_invoice);
            });
            ...
            $(this).find("input.release_booth").live('click', function() {
                ...
                $(this).jConfirmAction({
                    ...
                    cancelAnswer: i18n.confirm.cancel,
                    callback: function() {
                        callshop.release_booth( booth.closest('tr.slide').prev() );
                    }
                });
            });
                                        ...
                                        if (current.id == booth.id){
                                            ...
                                            if ((current.timestamp != booth.timestamp) || (current.balance != booth.balance)){
                                                ...
                                                if (current.state.match(/occupied|reserved/)) {
                                                    ...
                                                    var num = httpGet(mor_web_dir+"/callshop/number_digits")
        ...
        var callshop = {
            ...
            /*
             Reset Booth action, imediately makes booth free, and starts new session using default values from DB
            */
            reset_booth: function(a_booth, withInvoice) {
                withInvoice = withInvoice || 0 // Param specifying weather invoice should be generated, default 0 - not
                var booth = elementToBooth(a_booth);

                dataForAction = 'user_id=' + booth.id + "&server=" + booth.server + "&channel=" + booth.channel + "&with_invoice=" + withInvoice;

                if (withInvoice) { // Rubyists shouldn't forget that 1 - true, 0 - false in JS
                  // In case Invoice should be generated additional details should be stored in params
                  dataForAction += invoiceDataForAction();
                }

                $.ajax({
                    url: options.urls.reset_booth,
                    type: 'post',
                    postType: 'json',
                    cache: false,
                    data: dataForAction,
            ...
            release_booth: function(a_booth) {
                ...
                $.ajax({
                    ...
                    type: 'post',
                    data: 'user_id=' + booth.id + dataForAction,
                    beforeSend: function() {
                        callshop.change_state(booth, "loading", {});
                    },
                    success: function(data) {
             ...
             */
            free_booth: function(a_booth) {
                var booth = elementToBooth(a_booth);

                if (booth.state != "topup" && booth.state != 'free') { /* do not open more than two forms for one booth */
                    $.ajax({
                ...
                } else if ( booth.state == 'free'){
                    this.reset_booth(a_booth);
                }
            },
        ...
        function invoiceDataForAction(){
            ...
            dataForAction = "&comment=" + comment + "&balance=" + balance + "&full_payment=" + payment_received;
