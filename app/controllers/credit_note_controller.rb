...
class CreditNoteController < ApplicationController
  ...
  def new
    ...
    flash[:notice] = _('There_is_no_users') unless @users

    @note = CreditNote.new(issue_date: Time.now)
  end

  def create
    if accountant?
      condition = ["owner_id = 0 AND id = ?", params[:user][:id].to_i]
    elsif reseller? or admin?
      condition = ["owner_id = #{current_user_id} AND id = ? ", params[:user][:id].to_i]
    end
    user = User.where(condition).first
    if user
      @note = CreditNote.create_by(user, params)
      if @note.save
...
=end
  def credit_notes_count
    CreditNote.includes(:user).where(credit_note_conditions.join(' AND ')).references(:user).to_a.size
  end
...
=end
  def credit_notes(limit, offset)
    ...
    CreditNote.select("credit_notes.*, #{SqlExport.nice_user_sql}, users.username, users.last_name, users.first_name").
               joins('LEFT JOIN users ON (users.id = credit_notes.user_id)').
...
=end
  def credit_note_conditions
    if accountant?
      condition = ["users.owner_id IN (0, #{current_user_id})"]
    elsif reseller? or admin?
      condition = ["users.owner_id = #{current_user_id}"]
    end
    ...
    if options[:status] == 'paid' and options[:paid_date_from] and options[:paid_date_till]
      condition << credit_note_conditions_period(options, :paid)
    end
    if options[:issue_date_from] and options[:issue_date_till]
      condition << credit_note_conditions_period(options, :issue)
    end
    return condition
  end

  def credit_note_conditions_period(options, type)
    from = type == :paid ? options[:paid_date_from] : options[:issue_date_from]
    till = type == :paid ? options[:paid_date_till] : options[:issue_date_till]
    ...
    return " #{type == :paid ? 'pay_date' : 'issue_date'} BETWEEN '#{from}' AND '#{till}'"
  end
...
=end
  def find_credit_note(credit_note_id)
    if accountant?
      CreditNote.includes(:user).where(['users.owner_id IN (0, ?) AND credit_notes.id = ?', current_user_id, credit_note_id.to_i]).references(:user).first
    elsif reseller? or admin?
      CreditNote.includes(:user).where(['users.owner_id = ? AND credit_notes.id = ?', current_user_id, credit_note_id.to_i]).references(:user).first
    end
  end
...
=end
  ...
  def can_view_credit_notes?
    if admin? or reseller? or (accountant? and current_user.accountant_allow_read('invoices_manage'))
      true
    else
      dont_be_so_smart
...
=end
  def can_edit_finances?
    current_user and (admin? or reseller? or (accountant? and current_user.accountant_allow_edit('see_financial_data') and current_user.accountant_allow_read('invoices_manage')))
  end
...
=end
  def can_view_finances?
    current_user and ((admin? or reseller? or (accountant? and current_user.accountant_allow_read('see_financial_data') and current_user.accountant_allow_read('invoices_manage'))))
  end
