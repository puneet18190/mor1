...
class BackupsController < ApplicationController
  ...
  def backup_download
    ...
    backup = Backup.where(id: params[:id]).first
    redirect_to_backup_manager = redirect_to action: :backup_manager
    unless backup
      flash[:notice] = _('Backup_was_not_found')
      (redirect_to_backup_manager) && (return false)
    end
  ...
  def backup_restore
    ...
    backup = Backup.where(id: params[:id]).first
    backup_not_found = _('Backup_was_not_found')
    unless backup
      flash[:notice] = backup_not_found
      redirect_to action: :backup_manager and return false
    end
