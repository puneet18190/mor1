...
class GatewayEngine
 ...
 def on_find
   # find owner
   owner = User.where(:id => @user).first.try(:owner_id)
   ...
   if confirmation.blank? or confirmation == "none"
     ...
     if Confline.get_value("Email_Sending_Enabled", 0).to_i == 1
       if gateway.get(:config, 'payment_notification').to_i == 1
         email = Email.where({:name => 'payment_notification_regular', :owner_id => user.owner_id}).first
 ...
 def on_after_google_checkout_successful_payment
   ...
   MorLog.my_debug "|||||||||||||||||||||||"
   MorLog.my_debug item.unit_price.method(:to_s).source_location
     item.quantity = 1
   end
   MorLog.my_debug "|||||||||||||||||||||||"
   MorLog.my_debug checkout_command.to_yaml
   MorLog.my_debug "|||||||||||||||||||||||"
   MorLog.my_debug checkout_command.to_xml
