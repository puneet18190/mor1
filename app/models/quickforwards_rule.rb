...
class QuickforwardsRule < ActiveRecord::Base
  attr_accessible :id, :name, :user_id, :rule_regexp, :created_at, :updated_at

  has_many :users, foreign_key: 'quickforwards_rule_id'
  belongs_to :user

  validates_presence_of :name, :message => _('Name_cannot_be_blank')
  validates_format_of :rule_regexp, :with => /\A[0-9\[\]\%\|\,]*\z/, :message => _('Invalid_regexp')
  ...
  def check_prefix_regexp
    begin
      QuickforwardsRule.where(["rule_regexp REGEXP ?", self.rule_regexp]).to_a
    rescue
  ...
  def check_collisions_with_dids
    ...
    dids = Did.includes(:dialplan).where("dids.did REGEXP('^(#{regexp})') AND (dialplans.dptype != 'quickforwarddids' OR (dialplans.id IS NULL AND status = 'reserved'))").references(:dialplan)
