# -*- encoding : utf-8 -*-
# Common Use Providers are Providers whose belongs to admin and can be used by Resellers PRO.
# Reseller will be billed using Provider assigned to him as common use.
...
=end
  ...
  def self.common_use_providers?(resellerA, resellerB)
    if resellerA.is_reseller? && resellerB.is_reseller?
      if resellerA.has_own_providers? || resellerB.has_own_providers?
        return false
      else
  ...
  def name_by_reseller
    name = self.provider_name.to_s
    name = "#{_('Provider')} #{self.provider_id}" if name.blank?
    name
  end
end
