...
class Lcr < ActiveRecord::Base
  ...
  has_many :lcr_timeperiods, :foreign_key => :main_lcr_id, :dependent => :destroy #  Lcr.find(id).lcr_timeperiods returns owned set of periods, not which it takes part in.
  belongs_to :user
  has_many :lcrproviders #, :dependent => :destroy

  attr_accessor :main_lcr_id

  before_destroy :validate_before_destroy
  after_save :equalize_percent, :create_timeperiods

  def validate_before_destroy

    if User.where(["lcr_id = ?", id]).all.size > 0
      errors.add(:users, _('LCR_not_deleted_because_it_is_used_by_some_user(s)'))
      return false
    end

    lrules= Locationrule.where("lcr_id='#{id}'").all
    ...
    alert = Alert.where("action_alert_change_lcr_id = #{id} OR action_clear_change_lcr_id = #{id}").first
    if alert
      errors.add(:alerts, _('LCR_not_deleted_because_it_is_used_by_some_alerts'))
      return false
    end
  end
  ...
  def active_providers
    Provider.find_by_sql ["SELECT providers.* FROM providers, lcrproviders WHERE providers.id = lcrproviders.provider_id AND providers.active = 1 AND lcrproviders.active = 1 AND lcrproviders.lcr_id = ? AND providers.hidden = 0 ORDER BY providers.name ASC", self.id]
  end

  def add_provider(prov)
    unless Lcrprovider.where('lcr_id = ? AND provider_id = ?', self.id, prov.id).first
      if self.order.to_s == "percent"
        @prs = Lcrprovider.where(["lcr_id = ?", self.id]).all
        if @prs
        ...
        lprov = Lcrprovider.where(['lcr_id = ?', self.id]).order('priority desc').first
        Lcrprovider.create({lcr_id: id, provider_id: prov.id.to_i, priority: (lprov ? lprov.priority.to_i + 1 : 1)})
      end
    else
      errors.add(:add_provider, _('Provider_already_assign'))
      return false
    end
  end
  ...
  def remove_provider(prov_id)
    if self.order.to_s == "percent"
      @pr2 = Lcrprovider.where(["lcr_id = ? AND provider_id = ?", self.id.to_s, prov_id.to_s]).first
      if @pr2
        @prs = Lcrprovider.where(["lcr_id = ? AND id != ?", self.id.to_s, @pr2.id]).all
        if @prs.size > 0
  ...
  def lcr_partials_destinations(options = {})

    option_s_destination = replace_forward_slash(options[:s_destination])
    option_s_prefix = replace_forward_slash(options[:s_prefix])
    option_s_country = options[:s_country]

    search = []
    search << " AND (destinations.name LIKE '#{option_s_destination}' OR lcr_partials.destination_name LIKE '#{option_s_destination}')" if option_s_destination.present?
    search << " AND destinations.prefix LIKE '#{option_s_prefix}'" if option_s_prefix.present?
    search << " AND (destinations.direction_code = '#{option_s_country}' OR dst.direction_code = '#{option_s_country}')" if option_s_country.present?
    order_by = options[:order_by].present? ? "#{options[:order_by]} #{options[:order_desc].to_i == 0 ? 'ASC' : 'DESC'}" : 'name ASC'

    sql = "SELECT directions.*, lcr_partials.id AS 'lid', lcr_partials.prefix AS 'prefix', lcr_partials.lcr_id,
      lcrs.name AS 'lname', lcrs.order, lcrs.id AS 'lcr_id', destinationgroups.name AS 'dstgroup_name',
      IF(LENGTH(lcr_partials.destination_name) > 0, lcr_partials.destination_name, destinations.name) AS 'dest_name',
      destinationgroups.id AS 'destinationgroup_id'
      FROM lcr_partials
      LEFT JOIN lcrs on (lcrs.id = lcr_partials.lcr_id)
      LEFT JOIN destinations on (destinations.prefix = lcr_partials.prefix)
      LEFT JOIN destinations AS dst ON (dst.destinationgroup_id = lcr_partials.destinationgroup_id AND dst.destinationgroup_id > 0)
      LEFT JOIN destinationgroups on (lcr_partials.destinationgroup_id = destinationgroups.id)
      LEFT JOIN directions on (destinations.direction_code = directions.code OR dst.direction_code = directions.code)
      WHERE main_lcr_id = '#{self.id}' #{search.join}
      GROUP BY lcr_partials.id
      ORDER BY #{order_by}, lcr_partials.prefix ASC"
    MorLog.my_debug sql
    return ActiveRecord::Base.connection.select_all(sql)
  end

  def replace_forward_slash(option)
     option.gsub('\\', '\\\\\\\\') if option.present?
  end

  def equalize_percent
    if order_changed? and order == "percent"
      providers = Lcrprovider.select("lcrproviders.*").
          joins("RIGHT JOIN providers ON (providers.id = lcrproviders.provider_id)").
          where(["lcr_id = ?", self.id]).all
  ...
  def Lcr.lcrs_order_by(params, options)
    case params[:order_by].to_s.strip
      ...
      when "order"
        ...
        options[:order_by] ? order_by = "lcrs.#{options[:order_by]}" : order_by = "lcrs.name"
    end
    order_by += " ASC" if options[:order_desc].to_i == 0 and !order_by.blank?
    order_by += " DESC" if options[:order_desc].to_i == 1 and !order_by.blank?
  ...
  def provider_change_status(prov_id)
    ...
    Lcrprovider.where(['lcr_id = ? AND provider_id = ?', id, prov_id]).update_all("active = #{value}")
    return value
  end
  ...
  def make_tariff(options={})
    ...
    sql = "SELECT dir_name, name, prefix,
    ...
    if options[:test].to_i == 1
      ...
      return filename, mysql_res.to_json
    else
...
=end
  def self.clone_lcrs(resellerA, resellerB, lcr_list)
    if not CommonUseProvider.common_use_providers?(resellerA, resellerB)
      ...
      resellerA_lcrs = Lcr.where([query_condition]).all
      if resellerA_lcrs
...
=end
  ...
  def create_timeperiods
    # A: Create default set of timeperiods for Lcr if not present.
    timeperiods = self.lcr_timeperiods
    if timeperiods.size < 5
      n = 5 - timeperiods.size
      n.times do
#        new_period		= LcrTimeperiod.new
#        new_period.main_lcr_id	= self.id.to_i
#        new_period.name		= ''
        new_period = LcrTimeperiod.create({main_lcr_id: id.to_i, name: ''})
      end
    end
  end
