  ...
  begin
    ...
    class User < ActiveRecord::Base
      ...
      def find_actions_sum
        actions = Action.select('SUM(data2) as a_sum').where(['action=? AND DATE(date) = ? AND user_id=?', 'subscription_paid', M_date.to_date, self.id]).group('user_id').to_a
      ...
      def find_payments_sum
        actions = Payment.select('SUM(gross) as a_sum').where(['paymenttype=? AND DATE(shipped_at) = ? AND DATE(date_added) = ? AND user_id=?', 'subscription', M_date.to_date, M_date.to_date, self.id]).group('user_id').to_a
        if actions and actions.size.to_i > 0
          return actions[0].a_sum.to_f
        else
          return 0.to_f
      ...
      def subscriptions_in_period(period_start, period_end)
        ...
        subs = Subscription.includes([:service]).where(["(? BETWEEN activation_start AND activation_end OR ? BETWEEN activation_start AND activation_end OR (activation_start > ? AND activation_end < ?)) AND subscriptions.user_id = ?", period_start, period_end, period_start, period_end, self.id]).references([:service]).to_a
        prices = 0.to_f
    ...
    class Confline < ActiveRecord::Base
      def Confline.check_colldown
        c_time = Confline.where('name = "monthly_actions_cooldown_time"').first
