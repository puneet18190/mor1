...
begin
  ...
  class User < ActiveRecord::Base
    ...
    def payments_sum(def_curr, time = 0)
      ...
      #  puts cond
      pai = Payment.select("SUM(IF(payments.currency != '#{def_curr}', ((amount-tax) / currencies.exchange_rate), (amount-tax))) AS p_sum").
          joins("LEFT join currencies ON (payments.currency = currencies.name)").
          where(["user_id = ? and completed = 1 #{cond} and paymenttype in ('card', 'invoice', 'manual', 'voucher')", id]).to_a
      ...
      pai2 = Payment.select("SUM(IF(payments.currency != '#{def_curr}',gross / currencies.exchange_rate,gross)) AS p_sum").
          joins("LEFT join currencies ON (payments.currency = currencies.name)").
          where(["user_id = ? and completed = 1 #{cond} and paymenttype not in ('card', 'invoice', 'manual', 'voucher')", id]).to_a
    ...
    def sum_subscriptions
      s = Subscription.where(['user_id=?', id]).order('activation_start asc').first
      if s
    ...
    def if_action
      Action.where(['user_id=? and action="user_balance_at_month_end"', id]).order('id desc').first
    end

    def if_sub
      Subscription.where(['user_id=? and added >=?', id, Time.now.beginning_of_month.to_s(:db)]).to_a.size > 0
    end

    def sum_sub
      Subscription.select("sum(services.price) as s_sum").
          joins("Left JOIN services on (services.id = service_id)").
          where(['user_id=? and activation_start >=?', id, Time.now.beginning_of_month.to_s(:db)]).to_a.s_sum
    end
  ...
  users = User.where('blocked=0').to_a
