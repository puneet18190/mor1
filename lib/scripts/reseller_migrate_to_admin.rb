...
begin
  ...
  class Call < ActiveRecord::Base
    ...
    def Call.insert_split(names, names2, range_start)
      actions = Call.select("#{names}").joins('LEFT JOIN providers ON (providers.id = calls.provider_id)').where("(calls.reseller_id = #{Reseller_ID} OR calls.user_id = #{Reseller_ID} OR calls.dst_user_id = #{Reseller_ID})").group("calls.id").limit("#{range_start},1000").to_a
      sql_values = []
  ...
  class Confline < ActiveRecord::Base
    def Confline.update_velues
      confs = Confline.where({:owner_id => Reseller_ID}).to_a
  ...
  class Email < ActiveRecord::Base
    def Email.update_velues
      confs = Email.where(['owner_id=?', Reseller_ID]).to_a
  ...
  class Provider < ActiveRecord::Base
    def Provider.create_new
      ...
      MyWritter.msg "INSERT INTO devices (id, name, device_type, user_id, extension) VALUES (#{dev_id}, 'new_provider_from_migration', 'SIP', -1, '#{Device.first.extension}');"
      return p_id
    end
  end
      ...
      tables.each { |t|
        case t.to_s
          when 'Acustratedetail'
            actions = t.select("#{t.table_name}.*").joins("JOIN customrates ON (customrates.id = customrate_id)").where(" user_id IN (#{u_id})").group("#{t.table_name}.id").to_a if !u_id.blank?
          when 'Adaction'
            actions = t.select("#{t.table_name}.*").joins("JOIN campaigns ON (campaigns.id = campaign_id)").where("user_id IN (#{ru_id})").group("#{t.table_name}.id").to_a if !ru_id.blank?
          when *['Aratedetail', 'Ratedetail']
            actions = t.select("#{t.table_name}.*").joins("JOIN rates ON (rates.id = rate_id) JOIN tariffs ON (tariffs.id = rates.tariff_id)").where("owner_id = #{Reseller_ID}").group("#{t.table_name}.id").to_a
          when *['Callerid', 'Devicecodec', 'Callflow', 'Pdffaxemail', 'Pdffaxe']
            actions = t.select("#{t.table_name}.*").joins("JOIN devices ON (devices.id = device_id)").where("devices.user_id IN (#{ru_id})").group("#{t.table_name}.id").to_a if !ru_id.blank?
          when 'Extline'
            actions = t.select("#{t.table_name}.*").joins("LEFT JOIN devices ON (devices.id = device_id)").where("devices.user_id IN (#{ru_id}) OR device_id = 0").group("#{t.table_name}.id").to_a if !ru_id.blank?
          when 'VoicemailBox'
            actions = t.select("#{t.table_name}.*").joins("JOIN devices ON (devices.id = device_id)").where("devices.user_id IN (#{ru_id})").group("#{t.table_name}.uniqueid").to_a if !ru_id.blank?
          when *['CcGhostminutepercent', 'Cclineitem']
            actions = t.select("#{t.table_name}.*").joins("JOIN cardgroups ON (cardgroups.id = cardgroup_id)").where("cardgroups.owner_id = #{Reseller_ID}").group("#{t.table_name}.id").to_a
          when 'Ccorder'
            actions = t.select("#{t.table_name}.*").joins("JOIN cc_invoices ON (cc_invoices.ccorder_id = ccorders.id)").where("cardgroups.owner_id = #{Reseller_ID}").group("#{t.table_name}.id").to_a
          when *['Didrate', 'Quickforwarddid']
            actions = t.select("#{t.table_name}.*").joins("JOIN dids ON (dids.id = did_id)").where("dids.reseller_id = #{Reseller_ID}").group("#{t.table_name}.id").to_a
          when 'FlatrateData'
            actions = t.select("#{t.table_name}.*").joins("JOIN subscriptions ON (subscriptions.id = subscription_id)").where("subscriptions.user_id IN (#{u_id})").group("#{t.table_name}.id").to_a if !u_id.blank?
          when *['FlatrateDestination', 'Subscription']
            actions = t.select("#{t.table_name}.*").joins("JOIN services ON (services.id = service_id)").where("owner_id = #{Reseller_ID}").group("#{t.table_name}.id").to_a
          when 'Invoicedetail'
            actions = t.select("#{t.table_name}.*").joins("JOIN invoices ON (invoices.id = invoice_id)").where("user_id IN (#{ru_id})").group("#{t.table_name}.id").to_a if !ru_id.blank?
          when *['IvrAction', 'IvrExtension']
            actions = t.select("#{t.table_name}.*").joins("JOIN ivr_blocks ON (ivr_blocks.id = ivr_block_id) JOIN ivrs ON (ivrs.id = ivr_id)").where("user_id = #{Reseller_ID} ").group("#{t.table_name}.id").to_a
          when 'IvrBlock'
            actions = t.select("#{t.table_name}.*").joins("JOIN ivrs ON (ivrs.id = ivr_id)").where("user_id = #{Reseller_ID} ").group("#{t.table_name}.id").to_a
          when *['Lcrprovider', 'Providercodec', 'Providerrule', 'Serverprovider']
            actions = t.select("#{t.table_name}.*").joins("JOIN providers ON (providers.id = provider_id)").where("user_id = #{Reseller_ID} ").group("#{t.table_name}.id").to_a
          when 'Locationrule'
            actions = t.select("#{t.table_name}.*").joins("JOIN locations ON (locations.id = location_id)").where("user_id = #{Reseller_ID} ").group("#{t.table_name}.id").to_a
          when 'Rate'
            actions = t.select("#{t.table_name}.*").joins("JOIN tariffs ON (tariffs.id = tariff_id)").where("owner_id = #{Reseller_ID} ").group("#{t.table_name}.id").to_a
          when 'Tax'
            actions = t.select("#{t.table_name}.*").joins(" LEFT JOIN cs_invoices ON (cs_invoices.tax_id = taxes.id) LEFT JOIN invoices ON (invoices.tax_id = taxes.id) LEFT JOIN users ON (users.tax_id = taxes.id) LEFT JOIN vouchers ON (vouchers.tax_id = taxes.id) LEFT JOIN cardgroups ON (cardgroups.tax_id = taxes.id)").where("invoices.user_id IN (#{ru_id}) OR cs_invoices.user_id  IN (#{ru_id}) OR users.id  IN (#{ru_id}) OR vouchers.user_id IN (#{ru_id}) OR cardgroups.owner_id = #{Reseller_ID}").group("#{t.table_name}.id").to_a if !ru_id.blank?
          when 'Usergroup'
            actions = t.select("#{t.table_name}.*").joins("JOIN groups ON (groups.id = group_id)").where("owner_id = #{Reseller_ID} ").group("#{t.table_name}.id").to_a
          when 'Right'
            actions = t.select("#{t.table_name}.*").group("controller, action").to_a
          else
            actions = t.all.to_a
        end
      ...
      tables.each { |t|
        case t.to_s
          when *['Action', 'Campaign', 'Customrate', 'Devicegroup', 'Dialplan', 'Invoice', 'IvrSoundFile', 'IvrTimeperiod', 'IvrVoice', 'Ivr', 'LcrPartial', 'Lcr', 'Payment', 'Phonebook', 'Provider', 'Terminator', 'UserTranslation', 'Voucher']
            actions = t.select("#{t.table_name}.*").where("user_id IN (#{u_id})").group("#{t.table_name}.id").to_a   if !u_id.blank?
          when 'Device'
            actions = t.select("#{t.table_name}.*").joins('LEFT JOIN providers ON (providers.device_id = devices.id)').where("devices.user_id IN (#{u_id}) OR providers.user_id = #{Reseller_ID}").group("#{t.table_name}.id").to_a        if !u_id.blank?
          when 'Address'
            actions = t.select("#{t.table_name}.*").where("id IN (SELECT address_id FROM users WHERE id IN (#{u_id}))").group("#{t.table_name}.id").to_a    if !u_id.blank?
          when *['Cardgroup', 'Card', 'Group', 'Service', 'Tariff']
            actions = t.select("#{t.table_name}.*").where("owner_id = #{Reseller_ID}").group("#{t.table_name}.id").to_a
          when *['CsInvoice', 'CcInvoice']
            actions = t.select("#{t.table_name}.*").where("owner_id IN (#{u_id})").group("#{t.table_name}.id").to_a   if !u_id.blank?
          when 'User'
            actions = t.select("#{t.table_name}.*").where("id IN (#{u_id})").group("#{t.table_name}.id").to_a    if !u_id.blank?
          when 'Did'
            actions = t.select("#{t.table_name}.*").where("reseller_id = #{Reseller_ID}").group("#{t.table_name}.id").to_a
        end
        Debug.debug("Checking : #{t}")
        if actions and actions.size.to_i > 0
          msg = "#=================== DELETE AND INSERT INTO #{t.table_name} , found to insert : #{actions.size.to_i} ==========="
  ...
  users = User.where("owner_id = #{Reseller_ID}").to_a
  u_ids = []
