...
=end
    ...
    def Generate.generate_personal_rates(pdf, dgroups, tariff, tax, usr, options)
      ...
      pdf.table(items,
                :width => 550, :border_width => 0,
                :font_size => 9,
                :headers => [_('Name'), _('Rate'), _('Rate_with_VAT')],
                :align_headers => {0 => :left, 1 => :right, 2 => :right}) do
    ...
    def Generate.generate_user_rates_pdf(pdf, rates, tariff, options)
      ...
      for rate in rates
        item = []
        arate_details, arate_cur = Rate.get_user_rate_details(rate, 1)
        ...
        if arate_details.size > 0
          ...
          arate_cur += ' *' if arate_details.size > 1
          item << arate_cur
    ...
    def Generate.generate_cc_invoice(invoice, options)
      ...
      line_items = Cclineitem.includes([:cardgroup]).where("ccorder_id = #{options[:order].id}").references([:cardgroup]).to_a
  ...
  def Generate.generate_last_calls_pdf(calls, total_calls, current_user, main_options={})
    ...
    for call in calls
      ...
      if ['admin', 'accountant'].include?(usertype)
        ...
        item << call['user'].to_s.force_encoding('UTF-8')
        if main_options[:can_see_finances]
        ...
        if current_user.show_billing_info == 1 and main_options[:can_see_finances]
          if usertype == 'reseller'
            ...
            if User.current.own_providers.to_i == 0
              item << nice_number(call['self_cost_rate'], {:nice_number_digits => digits, :change_decimal => cgnd, :global_decimal => gnd})
              item << nice_number(call['self_cost_price'], {:nice_number_digits => digits, :change_decimal => cgnd, :global_decimal => gnd})
            end
